                        ...
                        response.data.forEach(function(item, i, arr) {
                            ...
                            that.list.forEach(function(item1, i1, arr1){
                                if(item1.game == item.game){
                                    that.list.events.push({id: item._id, game: item.game, name: item.name, city: item.city, likes: item.rating, date: item.date });
                                    found = true;
                                }
                            })
                            if(found == false){
                                //var subtype = typeArray[1];// ? typeArray[1];
                                that.list.push({game: item.game, events: [{id: item._id, game: item.game, name: item.name, city: item.city, likes: item.rating, date: item.date }]});
                            }
                        });
                      })
